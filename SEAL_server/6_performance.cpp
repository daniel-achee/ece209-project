...
std::string save_into_string(PublicKey const& pub) {
    std::ostringstream s;
    pub.save(s);
    return s.str();
}

void load_from_string(std::shared_ptr<SEALContext> context, PublicKey & pub, std::string str) {
    std::istringstream s;
    s.str(str);
    pub.load(context, s);
}
...
void bfv_performance_test(shared_ptr<SEALContext> context, int sockfd)
{
    ...
    char * cipherstr= new char[atoi(buf) + 1];
    bzero(cipherstr, atoi(buf) + 1);
    n = read(sockfd, cipherstr,atoi(buf));
     if (n < 0) error("ERROR reading from socket");
    
    Ciphertext encrypted1(context);
    Ciphertext encrypted2(context);

    load_from_string(context, encrypted1, cipherstr);

    bzero(buf, 255);
    n = read(sockfd,buf,255);
     if (n < 0) error("ERROR reading from socket");
    char * cipherstr2= new char[atoi(buf)+1];
    bzero(cipherstr2, atoi(buf) + 1);
    n = read(sockfd, cipherstr2,atoi(buf));
     if (n < 0) error("ERROR reading from socket");

    load_from_string(context, encrypted2, cipherstr2);
...
*/
    ...
    */
        ...
        */
       /*
        Ciphertext encryptedx(context);
        time_start = chrono::high_resolution_clock::now();
        encryptor.encrypt(plain, encryptedx);
        time_end = chrono::high_resolution_clock::now();
        time_encrypt_sum += chrono::duration_cast<
            chrono::microseconds>(time_end - time_start);
        string s =save_into_string(encryptedx);
        Ciphertext encrypted(context);
        load_from_string(context, encrypted, s);

        cout << "Serialized size: " << s.size();
        */
        /*
        [Decryption]
    ...
    */
        ...
        */
        ...
        string sum = save_into_string(encrypted1);
        n = write(sockfd,sum.c_str(), sum.size());
...
void example_bfv_performance_custom(int sockfd)
{
